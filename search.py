...
def search_files(directory, search_term: str, search_names_only=False, exact=False, recursive=True, word_distance=0) -> None:
    ...
    for root, _, files in iterable:
        for full_path in files:
            ...
            #Search File Name
            if result := dst_search_text(full_path, search_term, word_distance):
              ...
              try:
                  with open(full_path, 'r', encoding='utf-8') as f:
                      for line_number, line in enumerate(f, 1):
                          if result := dst_search_text(line, search_term, word_distance):
...
def dst_search_text(text: str, search_term: str, word_distance: int) -> List[Tuple[int, int]]:
    """
    ...
    for interval, window in window_gen:
        if difflib.SequenceMatcher(None, window, search_term).ratio() >= (1 - word_distance * 0.1):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--exact", action="store_true", help="Disable recursive search (default: recursive).")
    parser.add_argument("-d", "--distance", type=int, default=0, help="Set word match distance (0 for exact word match, 1 or more for allowing inexact matches, applies to each word).")
    ...
    search_files(args.directory, args.search_term, args.names_only, args.exact, args.recursive, args.distance)

"""This is my code so far.
Everything works nicely, but it is too difficult to read the console message, especially if there are multiple matches within each line.
I want to highlight each matching sequence in green"""