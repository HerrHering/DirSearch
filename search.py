...
def search_files(directory, search_term: str, search_names_only=False, recursive=True, tolerance=0) -> None:
    ...
    for root, _, files in iterable:
        for full_path in files:
            ...
            #Search File Name
            if result := dst_search_text(full_path, search_term, tolerance):
              ...
              try:
                  with open(full_path, 'r', encoding='utf-8') as f:
                      for line_number, line in enumerate(f, 1):
                          if result := dst_search_text(line, search_term, tolerance):
...
def dst_search_text(text: str, search_term: str, tolerance: int) -> List[Tuple[int, int]]:
    """
    ...
    # else
    matches = []
    window_gen = (((i, i + len(search_term) - 1), text[i:i + len(search_term)]) for i in range(len(text) - len(search_term) + 1))
    for interval, window in window_gen:
        if difflib.SequenceMatcher(None, window, search_term).ratio() >= (1 - tolerance * 0.01):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--tolerance", type=int, default=0, help="Set matching tolerance percentage (0 means exact match character by character, while anything > 0 gives a tolerance for words being 'similar enough')")
    ...
    search_files(args.directory, args.search_term, args.names_only, args.recursive, args.tolerance)
